...
$owners = ["6725145647", "7356540749"];  // Add owner ids here
...
if (isset($update['message']['text'])) {
    ...
    if ($message === '/users') {
        if (in_array($chat_id, $owners)) {
            ...
            $response = "<b>Total users: {$freeUserCount}%0APaid users: {$paidUserCount}%0ABanned users: {$banUserCount}%0A%0ABot by: @Xclpk</b>";
        } else {
